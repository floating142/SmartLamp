...
- **网络与集成**
	...
	- MQTT（轻量消息总线）：发布 `<base>/state`、订阅 `<base>/set`，上报 `<base>/availability`；支持 LWT、QoS=1 与 retained
	- Home Assistant：支持 MQTT Discovery，上报 `device_info` 与 availability 以便自动注册实体
	- 建议：限制消息大小（避免大 JSON）、启用心跳与重连、将凭据/主题配置持久化；SRAM 紧张时谨慎启用 TLS
